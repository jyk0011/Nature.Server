...
namespace Nature.Service.Ashx
{
    ...
    public class Post : BaseAshx
    {
        ...
        public override void ProcessRequest(HttpContext context)
        {
            ...
            {
                case "data":    //客户数据
                    url = SsoInfo.ResourceURL + "/data/PostData.ashx" + query;
                    break;
                case "meta":    //元数据
                    url = SsoInfo.ResourceURL + "/data/PostMeta.ashx" + query;
                    break;
                 
                default:        //按照当前的网页地址转发
                    if (SsoInfo.ResourceURL != "http://" + context.Request.Url.Host + ":" + context.Request.Url.Port.ToString(CultureInfo.InvariantCulture))
                        url = SsoInfo.ResourceURL + context.Request.Url.PathAndQuery + query;
